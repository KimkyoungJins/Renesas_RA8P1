...
z
    ...
    // if the par is 0 then it will skipt the sons secure enter
    ...
    const bsp_delay_units_t bsp_delay_units = BSP_DELAY_UNITS_MILLISECONDS;         // 1000 milliseconds -> 1 second

    /* Set the blink frequency (must be <= bsp_delay_units / 2) */
    const uint32_t freq_in_hz = 1;   // -> 2초에 한 번이면 될 것이다. 
